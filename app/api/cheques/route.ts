...
async function list() {
  ...
  return { today, rows, counts: { pending: rows.filter(x=>x.cheque_status==="pending").length, dueToday: rows.filter(x=>x.due_today).length, overdue: rows.filter(x=>x.overdue).length, cleared: rows.filter(x=>x.cheque_status==="cleared").length } };
}
