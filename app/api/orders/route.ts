...
export async function GET() {
  try { const data=await snapshot();if("unauthorized" in data)return NextResponse.json({error:"Staff sign-in required"},{status:401});return NextResponse.json(data); }
...
export async function POST(request:Request){
  try{
    ...
    const member=await getCurrentMember(db);if(!member)return NextResponse.json({error:"Staff sign-in required"},{status:401});
